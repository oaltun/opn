...
class OptimizationAlgorithm( Default ):
	...
	def run( self ):
		...
		cnt = {'time':0, 'maxhei': float( '-inf' ), 'mincost':float( 'inf' ), 'iteration':0, 'assessmentcnt':0 }
		loop = self.search()  # call the actual search function
		while self.iscontinue( cnt, self.stop ):
			bestpos, maxhei, iteration = loop.next()
			cnt = {'time':time.time() - tstart, 'iteration':iteration, 'maxhei':maxhei, 'mincost':-maxhei, 'bestpos':bestpos, 'assessmentcnt':self.problem.assessmentcnt()}
			print "yield:", cnt
			self.log.append( cnt )
			if self.isdraw:
